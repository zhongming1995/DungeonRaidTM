...
@ccclass  /**毒气炸弹技能 */
export default class PoisonBottleSkill extends RoleSkill {
    ...
    playEff(){
        ...
        for(let i=0;i<this.launchNum;i++){
            this.castSkillEff({idx:i+1,target:this.atkTargets[i]});
        }    
    }

    getTargetByIdx(idx:number){
        return this.atkTargets[idx-1]
    }
