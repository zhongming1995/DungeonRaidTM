import Player from "../../logics/role/Player";
...
export default class Joystick extends cc.Component {
  ...
  setSize(w?:number,h?:number){
      w >=0 && (this.node.width =w);
      h >=0 && (this.node.height =h);
  }
  ...
  _touchMoveEvent(event: cc.Event.EventTouch) {
    ...
    //   moveDistance: p,
    // });
    this.handler &&  this.handler.onTouchMove(event,{speedType: speedType,moveDistance:p});
  }
  ...
  _touchEndEvent(event: cc.Event.EventTouch) {
    ...
    // });
    this.handler &&  this.handler.onTouchEnd(event,{speedType: SpeedType.STOP});
  }

  _handleEvent(e:cc.Event.EventTouch,params:any){
